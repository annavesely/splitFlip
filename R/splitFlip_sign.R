#' @title Resampling-Based Multisplit (signed)
#' @description This function computes resampling-based standardized scores for high-dimensional linear regression.
#' @usage splitFlip_sign(X, Y, Q = 50, B = 200, target = NULL, varSel = selLasso, varSelArgs = NULL, exact = FALSE, maxRepeat = 20, seed = NULL, sign_type = 0)
...
#' G1 <- splitFlip_sign(X, Y, target=target, seed=42, sign_type=0)
#'
#' # maxT algorithm
#' maxT(abs(G1), alpha=0.1)
#'
#' # standardized scores using the exact method with oracle selection of target variables
#' G2 <- splitFlip_sign(X, Y, target=target, varSel=selOracle, varSelArgs=list(toSel=active), seed=42, sign_type=0)
#'
#' # maxT algorithm
#' maxT(abs(G2), alpha=0.1)
#' @export


splitFlip_sign <- function(X, Y, Q=50, B=200, target=NULL, varSel=selLasso, varSelArgs=NULL, exact=FALSE, maxRepeat=20, seed=NULL, sign_type=0){
  ...
  sel <- getSplits_sign(X, Y, Q, target, varSel, varSelArgs, maxRepeat, seed)
  ...
  for(j in seq(m)){G[,j] <- jsplitFlip_sign(j, X, Y, sel, fl, exact, sign_type)}
  return(G)
}
