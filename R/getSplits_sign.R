#' @title Data Splits and Variable Selection (signed)
...
#' @usage getSplits_sign(X, Y, Q, target, varSel, varSelArgs, maxRepeat, seed)
...
#' @return \code{getSplits_sign} returns a list containing
#' \itemize{
...
#' \item \code{signs}: matrix containing, for each split (row) and each variable (column), the sign of the estimated coefficients
#' (this is 1 if the coefficient is non-negative, and 0 only when the variable is not selected)
#' }
#' @author Anna Vesely.
#' @noRd
...
getSplits_sign <- function(X, Y, Q, target, varSel, varSelArgs, maxRepeat, seed){
